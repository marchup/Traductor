...
import { translateToSanjotanes } from '../lib/sanjotanes'
...
export default function Translator() {
  ...
  return (
    <div className="space-y-6">
      <div className="panel">
        <label className="block text-gold-500 font-semibold mb-2">Texto en Español</label>
        <textarea
          value={input}
      ...
      {result && (
        <div className="panel border-gold-500/30">
          ...
          <div className="mb-4">
            ...
            <p className="text-lg">{result.original}</p>
          </div>
          <div>
            <label className="text-sm text-gold-600">Sanjotanes:</label>
            <p className="text-2xl font-mono text-gold-500">{result.translated}</p>
          </div>
          
          {result.words.length > 0 && (
            <div className="mt-6">
              <h4 className="text-sm font-semibold text-gray-400 mb-3">Detalle:</h4>
                ...
                <tbody>
                  {result.words.map((word, idx) => (
                    <tr key={idx} className="border-b border-ocean-700/50">
                      ...
                      <td className="py-2 font-mono text-gold-400">{word.sanjotanes}</td>
                      <td className="py-2">
                        {word.isNew ? (
                          <span className="text-green-400 text-xs bg-green-900/30 px-2 py-1 rounded">NUEVO</span>
                        ) : (
                          <span className="text-blue-400 text-xs bg-blue-900/30 px-2 py-1 rounded">LOCKED</span>
                        )}
                      </td>
                    </tr>
                  ))}
